...
COLOR_SHIELD_EMPTY = (60, 40, 50)
COLOR_COOLDOWN_TEXT = (255, 75, 75)
...
class Sword:
    def __init__(self):
        ...
        # Parry Charges & 12s Cooldown
        self.is_parrying = False
        self.is_holding_parry = False
        self.max_parry_charges = 3
        self.parry_charges = 3
        
        self.parry_cooldown_timer = 0
        self.max_parry_cooldown = 720  # 12s at 60 FPS
        
        self.parry_progress = 0.0
        self.shield_float_timer = 0.0
        self.shield_offset_x = -12.0
        self.cd_text_offset_x = 12.0

        # Widget Animation Progress (0.0 = Hidden, 1.0 = Visible)
        self.widget_anim_progress = 0.0

        self.particles = []
        self.current_tip = pygame.Vector2(0, 0)
        
        # Tiny Font for On-Player Countdown Text
        self.small_cd_font = pygame.font.SysFont("Arial", 9, bold=True)
        self.widget_font = pygame.font.SysFont("Arial", 11, bold=True)

    @property
    def is_attacking(self):
        return self.is_swinging or self.is_dashing

    def get_hitbox(self):
        if self.is_attacking:
            size = 20 if self.is_dashing else 16
            return pygame.Rect(self.current_tip.x - size // 2, self.current_tip.y - size // 2, size, size)
        return None
    ...
    def trigger_parry(self, player_pos, mouse_world_pos):
        if not self.is_swinging and not self.is_dashing and self.parry_charges > 0 and self.parry_cooldown_timer <= 0:
    ...
    def consume_parry_charge(self):
        if self.parry_charges > 0:
            self.parry_charges -= 1
            if self.parry_charges == 0:
                self.parry_cooldown_timer = self.max_parry_cooldown
                self.release_parry()
    ...
    def _draw_divided_shield_icon(self, surface, center_pos, alpha):
        w, h = 12, 14
        surf = pygame.Surface((w, h), pygame.SRCALPHA)

        outer_points = [(1, 0), (10, 0), (11, 4), (10, 9), (6, 13), (1, 9), (0, 4)]
        ...
        if self.parry_charges >= 1:
            pygame.draw.polygon(surf, (*COLOR_SHIELD_CORE, alpha), [(2, 2), (5, 2), (5, 6), (2, 6)])
        else:
            pygame.draw.polygon(surf, (*COLOR_SHIELD_EMPTY, alpha), [(2, 2), (5, 2), (5, 6), (2, 6)])

        if self.parry_charges >= 2:
            pygame.draw.polygon(surf, (*COLOR_SHIELD_CORE, alpha), [(6, 2), (9, 2), (9, 6), (6, 6)])
        else:
            pygame.draw.polygon(surf, (*COLOR_SHIELD_EMPTY, alpha), [(6, 2), (9, 2), (9, 6), (6, 6)])

        if self.parry_charges >= 3:
            pygame.draw.polygon(surf, (*COLOR_SHIELD_CORE, alpha), [(2, 7), (9, 7), (6, 11)])
        else:
            pygame.draw.polygon(surf, (*COLOR_SHIELD_EMPTY, alpha), [(2, 7), (9, 7), (6, 11)])

        surface.blit(surf, (center_pos.x - (w / 2), center_pos.y - (h / 2)))

    def draw_middle_right_cooldown(self, screen):
        """Displays smooth animated Parry Cooldown widget at middle-right corner."""
        target_anim = 1.0 if self.parry_cooldown_timer > 0 else 0.0
        self.widget_anim_progress += (target_anim - self.widget_anim_progress) * 0.12

        if self.widget_anim_progress > 0.01:
            screen_w = screen.get_width()
            screen_h = screen.get_height()

            seconds_left = math.ceil(self.parry_cooldown_timer / 60)
            text_str = f"Parry Cooldown: {seconds_left}s"

            text_surf = self.widget_font.render(text_str, True, COLOR_COOLDOWN_TEXT)
            
            # Box dimensions
            icon_w = 10
            padding_x = 8
            padding_y = 6
            box_w = icon_w + text_surf.get_width() + (padding_x * 3)
            box_h = 24

            # Animated Position: slides in smoothly from the right
            target_x = screen_w - box_w - 15
            start_x = screen_w + 10
            anim_x = int(start_x + (target_x - start_x) * self.widget_anim_progress)
            center_y = (screen_h // 2 + 50) - (box_h // 2)

            alpha_val = int(180 * self.widget_anim_progress)

            # Transparent Widget Box
            widget_box = pygame.Surface((box_w, box_h), pygame.SRCALPHA)
            widget_box.fill((18, 12, 18, alpha_val))
            pygame.draw.rect(widget_box, (180, 50, 60, int(200 * self.widget_anim_progress)), (0, 0, box_w, box_h), 1, border_radius=4)

            # Draw Mini Red Shield Icon on left inside widget
            shield_surf = pygame.Surface((10, 12), pygame.SRCALPHA)
            shield_pts = [(1, 0), (8, 0), (9, 3), (8, 7), (5, 11), (1, 7), (0, 3)]
            pygame.draw.polygon(shield_surf, (220, 50, 60, int(255 * self.widget_anim_progress)), shield_pts)
            pygame.draw.polygon(shield_surf, (255, 180, 180, int(255 * self.widget_anim_progress)), shield_pts, width=1)

            widget_box.blit(shield_surf, (padding_x, (box_h - 12) // 2))

            # Draw Cooldown Text
            text_copy = text_surf.copy()
            text_copy.set_alpha(int(255 * self.widget_anim_progress))
            widget_box.blit(text_copy, (padding_x + icon_w + padding_x, (box_h - text_surf.get_height()) // 2))

            screen.blit(widget_box, (anim_x, center_y))
    ...
    def update(self, player_rect, mouse_world_pos, move_player_func, walls):
        ...
        if self.parry_cooldown_timer > 0:
            ...
            if self.parry_cooldown_timer <= 0:
                self.parry_charges = self.max_parry_charges
        ...
        target_parry_progress = 1.0 if (self.is_parrying and self.is_holding_parry) else 0.0
        ...
        facing_angle = math.atan2(dir_to_mouse.y, dir_to_mouse.x) if dir_to_mouse.length() > 0 else 0
        ...
        if self.is_swinging:
            ...
            self.current_tip = hand_pos + pygame.Vector2(math.cos(current_sword_angle), math.sin(current_sword_angle)) * self.length

            self.particles.append(Particle(self.current_tip.x, self.current_tip.y, mode="SWING", angle=current_sword_angle))
            self.particles.append(Particle(self.current_tip.x, self.current_tip.y, mode="SLICE", angle=current_sword_angle + (math.pi / 2)))
            ...
            dash_angle = math.atan2(self.dash_dir.y, self.dash_dir.x)
            self.current_tip = player_center + pygame.Vector2(math.cos(dash_angle), math.sin(dash_angle)) * (self.length + 6)
            
            self.particles.append(Particle(player_center.x, player_center.y, mode="THRUST", angle=dash_angle + math.pi))

            self.dash_timer -= 1
            if self.dash_timer <= 0:
                self.is_dashing = False
        else:
            self.current_tip = player_center + pygame.Vector2(math.cos(facing_angle), math.sin(facing_angle)) * self.length
    ...
    def draw(self, surface, player_rect, mouse_world_pos):
        ...
        # Render Parry Block Stance & Shield Icon
        if self.parry_progress > 0.01:
            ...
            self._draw_divided_shield_icon(surface, icon_center, icon_alpha)
        ...
        # DYNAMIC SMALL RED TIMER (ONLY VISIBLE ON RMB HOLD WHEN ON COOLDOWN)
        keys_mouse = pygame.mouse.get_pressed()
        if keys_mouse[2] and self.parry_cooldown_timer > 0:
            target_cd_x = -14.0 if dir_to_mouse.x >= 0 else 14.0
            self.cd_text_offset_x += (target_cd_x - self.cd_text_offset_x) * 0.25

            seconds_left = math.ceil(self.parry_cooldown_timer / 60)
            cd_txt = self.small_cd_font.render(f"{seconds_left}s", True, COLOR_COOLDOWN_TEXT)
            
            txt_x = player_center.x + self.cd_text_offset_x - (cd_txt.get_width() // 2)
            txt_y = player_center.y - 10
            surface.blit(cd_txt, (txt_x, txt_y))
